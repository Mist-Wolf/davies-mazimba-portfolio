...
export function Education({ dark }: { dark: boolean }) {
  return (
    <section id="education" className="max-w-6xl mx-auto px-6 py-12 space-y-8">
      <div className="mb-8">
        <p className="text-sm uppercase tracking-[0.25em] text-violet-300">Learning & growth</p>
        <h2 className={`mt-2 text-4xl font-bold tracking-tight ${dark ? "text-white" : "text-slate-900"}`}>Education</h2>
      </div>
      <div className="space-y-6">
        {education.map((item: EducationItem) => (
          <motion.div
            ...
            key={item.school}
            className={`group relative p-6 rounded-2xl shadow-md border border-transparent hover:border-green-400 transition-all duration-300
              ${dark ? "bg-white/[0.07] hover:bg-white/[0.12]" : "bg-white hover:bg-gray-100"}`}
          >
